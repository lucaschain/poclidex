...
describe("GenerationService", () => {
  ...
  describe("setSessionGeneration()", () => {
    ...
    it("should throw error for generation 0", () => {
      expect(() => service.setSessionGeneration(0)).toThrow(
        "Invalid generation: 0",
      );
    });

    it("should throw error for generation 10", () => {
      expect(() => service.setSessionGeneration(10)).toThrow(
        "Invalid generation: 10",
      );
    });

    it("should throw error for negative generations", () => {
      expect(() => service.setSessionGeneration(-1)).toThrow(
        "Invalid generation: -1",
      );
    });

    it("should throw error with correct message format", () => {
      expect(() => service.setSessionGeneration(100)).toThrow(
